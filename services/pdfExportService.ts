...
import { Course, CalculationResult, HistoryEntry, GPASettings } from '../types';
...
export const exportGpaReportAsPdf = (
  ...
  if (settings.classRank && settings.classSize && settings.classSize > 0) {
    const percentile = Math.ceil((settings.classRank / settings.classSize) * 100);
    summaryBody.push(['Class Rank', `${settings.classRank} / ${settings.classSize} (Top ${percentile}%)`]);
  }
